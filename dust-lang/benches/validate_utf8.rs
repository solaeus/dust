...
use dust_lang::validate_utf8_and_find_token_spans;
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("all valid utf8", |b| {
        b.iter(|| validate_utf8_and_find_token_spans(black_box(&all_valid_utf8)))
    });

    let all_ascii = Vec::from_iter(0u8..=127);
    let big_ascii = all_ascii.repeat(100_000);

    group.bench_function("full ascii repeated 100,000 times", |b| {
        b.iter(|| validate_utf8_and_find_token_spans(black_box(&big_ascii)))
    });

    let bigger_ascii = all_ascii.repeat(1_000_000);

    group.bench_function("fill ascii repeated 1,000,000 times", |b| {
        b.iter(|| validate_utf8_and_find_token_spans(black_box(&bigger_ascii)))
    });
    group.finish();
}
