...
#[test]
fn empty() {
    let parser = Parser::new(SourceFileId::MAIN, Lexer::with_unvalidated_source(b"fn foo() {}"));
    let ParseResult {
        syntax_tree,
        errors,
        ..
...
#[test]
fn return_type() {
    let parser = Parser::new(SourceFileId::MAIN, Lexer::with_unvalidated_source(b"fn foo() -> i64 {}"));
    let ParseResult {
        syntax_tree,
        errors,
        ..
