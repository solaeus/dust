...
impl<'a> ChunkCompiler<'a> {
    ...
    fn declare_local(
        ...
        trace!(
            ...
            address.to_string(r#type.as_operand_type())
        );
    ...
    fn get_last_expression_operation(&self) -> Option<Operation> {
    ...
    fn get_last_instruction_operations<const COUNT: usize>(&self) -> Option<[Operation; COUNT]> {
    ...
    fn emit(
        ...
        self.expressions.push(Expression {
            ...
            r#type,
            ends_with_value: ends_statement,
            scope: self.current_block_scope,
            position,
        });
    ...
    fn parse_boolean(&mut self) -> Result<(), CompileError> {
        ...
        if let Token::Boolean(text) = self.current_token {
            ...
            self.emit(load, ExpressionKind::Literal, Type::Boolean, true, position);
    ...
    fn parse_unary(&mut self) -> Result<(), CompileError> {
        ...
        let instruction = match operator {
            ...
            Token::Minus => {
                let operand_type = previous_type.as_operand_type();
    ...
    fn parse_comparison_binary(&mut self) -> Result<(), CompileError> {
        ...
        ) = self.get_last_instruction_operations()
            && matches!(
                self.previous_token,
                Token::DoubleEqual
                    | Token::BangEqual
    ...
    fn parse_logical_binary(&mut self) -> Result<(), CompileError> {
        ...
        let left_instruction = self.instructions[left_instruction_index];
        let left_destination = left_instruction.destination();
        ...
        if matches!(
            ...
            let Load {
                ...
                jump_next,
                ..
            } = Load::from(*load_instructions[0]);
            *load_instructions[0] = Instruction::load(
            ...
            let Load {
                ...
                jump_next,
                ..
            } = Load::from(*load_instructions[1]);
            *load_instructions[1] = Instruction::load(
                Address::register(left_destination.index),
                operand,
                r#type,
                jump_next,
            ...
            if right_instruction.yields_value() {
                self.instructions
                    .last_mut()
                    .unwrap()
                    .set_destination(left_destination);
            }
        }
        ...
        if is_logic_chain {
            ...
            while instruction_slice[2].operation() == Operation::JUMP {
                if comparator {
                ...
                comparator = instruction_slice[1].a_field() != 0;
    ...
    fn parse_variable(&mut self) -> Result<(), CompileError> {
        ...
        let (expression_kind, ends_with_value) = if self.allow(Token::Equal)? {
            if !is_mutable {
            ...
            match index {
                ExpressionIndex::Instruction(instruction_index) => {
                    ...
                    if instruction.yields_value() {
                        instruction.set_destination(variable_address);
                    } else {
                        return Err(CompileError::ExpectedExpression {
                            found: self.previous_token.to_owned(),
                            position: expression_position,
    ...
    fn parse_block(&mut self) -> Result<(), CompileError> {
        ...
        let outer_scope = self.current_block_scope;
        ...
        self.current_block_scope.end(outer_scope.block_index);

        let end = self.previous_position.1;
        let end_expression_count = self.expressions.len();
        let block_type = self.expressions.last().map_or(Type::None, |expression| {
            if expression.ends_with_value {
                expression.r#type.clone()
            } else {
                Type::None
            }
        });
        let ends_with_value = block_type != Type::None;

        self.expressions.push(Expression {
            index: ExpressionIndex::Instruction(self.instructions.len() - 1),
        ...
        if self.get_last_instruction_operations() == Some([Operation::DROP]) {
            return Ok(());
        }

        let mut drop_list = self
            .locals
            .iter()
            .filter_map(|(_, local)| {
        ...
        for expression_index in starting_expression_count..end_expression_count {
            ...
            if expression_index == end_expression_count - 1 && *ends_with_value {
                // In this case, the value escapes the block scope, so it stays alive.
                continue;
            }

            let ExpressionIndex::Instruction(instruction_index) = index else {
                continue;
            };
            ...
            let destination_register = instruction.a_field();
            let Err(insertion_index) = drop_list.binary_search(&destination_register) else {
                continue;
            };
            ...
            drop_list.insert(insertion_index, destination_register);
        }

        if !drop_list.is_empty() {
            let drop_list_index = self.drop_lists.len() as u16;
            let drop = Instruction::drop(drop_list_index);

            self.drop_lists.push((block_scope, drop_list));
            self.instructions.push(drop);
        }
    ...
    fn parse_if(&mut self) -> Result<(), CompileError> {
        ...
        if matches!(
            ...
            let address_index = match last_expression_type {
        ...
        let (if_block_last_instruction, if_block_type) = if let Some(previous) = previous {
            if previous.yields_value() {
                let previous_active_register = previous.a_field();
            ...
            (previous, last_expression_type)
        } else {
            let last_instruction = self.instructions.last().cloned().unwrap();
            let last_expression_type = self
                .expressions
                .last()
                .ok_or_else(|| CompileError::ExpectedExpression {
            ...
            (last_instruction, last_expression_type)
        };
        ...
        let else_block_last_instruction = &mut self.instructions[else_block_last_instruction_index];
        ...
        match jump_distance {
            0 => {}
            1 => {
                if let Some([Operation::LOAD, _]) = self.get_last_instruction_operations() {
                    let load_index = self.instructions.len() - 2;
                    let load = self.instructions.get_mut(load_index).unwrap();
    ...
    fn parse_sub_expression(&mut self, precedence: &Precedence) -> Result<(), CompileError> {
        ...
        let expression_type = &self.expressions.last().unwrap().r#type;
    ...
    fn parse_implicit_return(&mut self) -> Result<(), CompileError> {
        if self.get_last_expression_operation() == Some(Operation::RETURN) {
            return Ok(());
        }
    ...
    fn parse_let(&mut self) -> Result<(), CompileError> {
        ...
        let Expression {
            index,
            r#type: last_instruction_type,
            position: last_instruction_position,
            ..
        } = self
            .expressions
            .last()
        ...
        let address = match index {
            ExpressionIndex::Instruction(instruction_index) => {
                self.instructions[instruction_index].destination()
            }
            ExpressionIndex::Function(prototype_index) => Address::constant(prototype_index as u16),
        };
    ...
    fn parse_function(&mut self) -> Result<(), CompileError> {
        ...
        let ends_statement = path.is_some();
    ...
    fn parse_call(&mut self) -> Result<(), CompileError> {
        ...
        let get_function_info = || -> Result<(Type, u16, usize), CompileError> {
            ...
            if let Token::Identifier(identifier) = function_token {
                ...
                if let Some(function) = prototypes.get_by_name(identifier) {
                    return Ok((
                        function.r#type.return_type.clone(),
                        function.prototype_index,
                        function.r#type.value_parameters.len(),
                    ));
                }
            }
