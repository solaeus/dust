...
fn local_boolean_less_than() {
    let source = create_function_case(local_cases::LOCAL_BOOLEAN_LESS_THAN);
...
fn local_byte_less_than() {
    let source = create_function_case(local_cases::LOCAL_BYTE_LESS_THAN);
    let prototypes = compile(source).unwrap();
...
fn local_character_less_than() {
    let source = create_function_case(local_cases::LOCAL_CHARACTER_LESS_THAN);
    let prototypes = compile(source).unwrap();
...
fn local_float_less_than() {
    let source = create_function_case(local_cases::LOCAL_FLOAT_LESS_THAN);
    let prototypes = compile(source).unwrap();
...
fn local_integer_less_than() {
    let source = create_function_case(local_cases::LOCAL_INTEGER_LESS_THAN);
    let prototypes = compile(source).unwrap();
...
fn local_string_less_than() {
    let source = create_function_case(local_cases::LOCAL_STRING_LESS_THAN);
    let prototypes = compile(source).unwrap();
