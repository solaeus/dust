...
use crate::{
    compiler::{
        context::{
            Context,
            declarations::{DeclarationId, Definition, FunctionKind, ModuleKind, VariantKind},
    ...
    syntax::{
        components::{
            ...
            TupleFields, TupleType, TypeItem, TypeParameter, UseItem, ValueParameter,
...
pub struct DeclarationResolver<'a, 'src> {
    ...
    worklist: &'a mut Vec<WorklistEntry>,

    forward_references: &'a mut Vec<DeclarationId>,
...
impl<'a, 'src> DeclarationResolver<'a, 'src> {
    ...
    pub fn visit_root(mut self, reader: SyntaxReader) -> Result<(), ErrorKind> {
    ...
    fn visit_mod_item(&mut self, reader: SyntaxReader) -> Result<(), CompileError> {
        ...
        let module_declaration_id = self.context.reserve_scoped_declaration_id(
            module_symbol_id,
    ...
    fn visit_use_item(&mut self, reader: SyntaxReader) -> Result<(), CompileError> {
        ...
        let file = self.source.get_code(path.code_id());
        ...
        let use_declaration_id = self.context.add_scoped_declaration(
            current_symbol_id,
            Definition::Use {
                public,
        ...
        self.context
            .add_declaration_binding(self.code_id, reader.id, use_declaration_id);
    ...
    fn visit_fn_item(&mut self, reader: SyntaxReader) -> Result<DeclarationId, CompileError> {
        ...
        let function_declaration_id = self.context.reserve_scoped_declaration_id(
            function_symbol_id,
        ...
        let (value_parameters_scope_id, function_kind) = if let Some(value_parameters) =
            value_parameters
        {
            let ValueParameters { parameters } = value_parameters.as_component()?;
            let mut function_kind = FunctionKind::Function;

            self.enter_scope(Barrier::Members);

            for parameter in parameters {
                match parameter.as_component()? {
                    ValueParameter::SelfParameter(self_parameter) => {
                        ...
                        let OuterDeclaration::Impl {
                            ...
                            return Err(CompileError::SelfTypeOutsideOfImpl {
                                position: value_parameters.position(),
                            });
                        };
                        let self_type_id =
                            if self_parameter.node.kind == SyntaxKind::SelfReferenceParameter {
                                self.context.types.add_type(Type::Reference {
                                    referenced_type_id: self_type_id,
                                })
                            } else {
                                self_type_id
                            };
                        ...
                        self.context.add_type_binding(
                            self.code_id,
                            self_parameter.id,
                            self_type_id,
                        );
                    }
                    ValueParameter::Named {
                        name,
                        type_notation,
                    } => {
                        let name_str = self.source.get_content(name.position())?;
                        let symbol_id = self.context.symbols.add_symbol(name_str);
                        let type_id = self.visit_type(type_notation)?;
                        let declaration_id = self.context.add_scoped_declaration(
                            symbol_id,
                            Definition::Local {
                                mutable: false,
                                shadowed: None,
                        ...
                        self.context
                            .add_declaration_binding(self.code_id, name.id, declaration_id);
                    }
                }
            }

            (Some(self.current_scope_id), function_kind)
        } else {
        ...
        self.context
            .add_declaration_binding(self.code_id, name.id, function_declaration_id);
    ...
    fn visit_struct_item(&mut self, reader: SyntaxReader) -> Result<(), CompileError> {
        ...
        let struct_declaration_id = self.context.reserve_scoped_declaration_id(
            struct_symbol_id,
            Some((reader.position(), reader.id)),
            self.current_scope_id,
        );
            ...
            match fields.node.kind {
                SyntaxKind::TupleFields => {
                    ...
                    for (index, field_type) in types.enumerate() {
                        ...
                        self.context.add_scoped_declaration(
                            field_symbol_id,
                            Definition::Field {
                                public,
                ...
                SyntaxKind::NamedFields => {
                    ...
                    for (field_name, field_type) in name_type_pairs {
                        ...
                        let field_type_id = self.visit_type(field_type)?;
                        let field_declaration_id = self.context.add_scoped_declaration(
                            field_symbol_id,
                            Definition::Field {
                                public,
                                parent_struct: struct_declaration_id,
        ...
        self.context
            .add_declaration_binding(self.code_id, reader.id, struct_declaration_id);
    ...
    fn visit_enum_item(&mut self, reader: SyntaxReader) -> Result<(), CompileError> {
        ...
        let enum_declaration_id = self.context.reserve_scoped_declaration_id(
            enum_symbol_id,
            Some((reader.position(), reader.id)),
            self.current_scope_id,
        );
        ...
        self.context
            .add_declaration_binding(self.code_id, name.id, enum_declaration_id);
    ...
    fn visit_type_parameters(
        ...
        for type_parameter in type_parameters_reader.children() {
            ...
            let type_parameter_declaration_id = self.context.add_scoped_declaration(
    ...
    fn visit_enum_variant(
        ...
        match reader.node.kind {
            SyntaxKind::EnumUnitVariant => {
                ...
                let variant_declaration_id = self.context.add_scoped_declaration(
                    variant_symbol_id,
                    Definition::Variant {
                        discriminant,
                        enum_declaration_id,
                ...
                self.context
                    .add_declaration_binding(self.code_id, name.id, variant_declaration_id);
            ...
            SyntaxKind::EnumTupleFieldsVariant => {
                ...
                let variant_name_str = self.source.get_content(name.position())?;
                let variant_symbol_id = self.context.symbols.add_symbol(variant_name_str);
                let variant_declaration_id = self.context.reserve_scoped_declaration_id(
                    variant_symbol_id,
                    Some((reader.position(), reader.id)),
                    self.current_scope_id,
                );
    ...
    fn visit_const_item(&mut self, reader: SyntaxReader) -> Result<DeclarationId, CompileError> {
        ...
        let const_declaration_id = self.context.add_scoped_declaration(
            const_symbol_id,
            Definition::Constant {
                public,
        ...
        self.context
            .add_declaration_binding(self.code_id, name.id, const_declaration_id);
    ...
    fn visit_type_item(&mut self, reader: SyntaxReader) -> Result<DeclarationId, CompileError> {
        ...
        let type_alias_declaration_id = self.context.add_scoped_declaration(
        ...
        self.context
            .add_declaration_binding(self.code_id, name.id, type_alias_declaration_id);
    ...
    fn visit_impl_item(&mut self, reader: SyntaxReader) -> Result<(), CompileError> {
        ...
        let impl_declaration_id = self.context.reserve_scoped_declaration_id(
            impl_symbol_id,
            Some((reader.position(), reader.id)),
            self.current_scope_id,
        );
        ...
        for child in body.children() {
            match child.node.kind {
                ...
                SyntaxKind::TypeItem => {
                    ...
                    let type_declaration_id = self.context.add_scoped_declaration(
                        type_symbol_id,
        ...
        self.context
            .add_declaration_binding(self.code_id, reader.id, impl_declaration_id);
        self.context
            .implementations
    ...
    fn visit_trait_item(&mut self, reader: SyntaxReader) -> Result<(), CompileError> {
        ...
        let trait_declaration_id = self.context.reserve_scoped_declaration_id(
            trait_symbol_id,
            Some((reader.position(), reader.id)),
            self.current_scope_id,
        );
        ...
        self.context
            .add_declaration_binding(self.code_id, name.id, trait_declaration_id);
    ...
    fn visit_range_expression(&mut self, reader: SyntaxReader) -> Result<(), CompileError> {
        ...
        self.context
            .add_declaration_binding(self.code_id, reader.id, declaration_id);
    ...
    fn visit_call_expression(&mut self, reader: SyntaxReader) -> Result<(), CompileError> {
        ...
        let callee_declaration_id = *self
            .context
            .get_declaration_binding(&(self.code_id, callee.id))?;

        self.context
            .add_declaration_binding(self.code_id, reader.id, callee_declaration_id);
    ...
    fn visit_method_call_expression(&mut self, reader: SyntaxReader) -> Result<(), CompileError> {
        ...
        if let Some(type_arguments) = type_arguments {
            for type_argument in type_arguments.children() {
                ...
                self.context
                    .add_type_binding(self.code_id, type_argument.id, type_id);
            }
        }
            ...
            SyntaxKind::TypePath => {
                ...
                match declaration.definition {
                    ...
                    Definition::TraitAssociatedType {
                        ...
                        let first_declaration_id = *self
                            .context
                            .get_declaration_binding(&(self.code_id, first_segment.id))?;
    ...
    fn visit_path(&mut self, path: SyntaxReader) -> Result<DeclarationId, CompileError> {
        ...
        self.context
            .add_declaration_binding(self.code_id, path.id, current_declaration_id);
...
pub struct WorklistEntry {
    pub module_code_id: CodeId,
    pub module_declaration_id: DeclarationId,
