...
impl<'a> Call<'a> {
    ...
    fn get_i8(&self, Address { memory, index }: Address) -> Result<i8, VmError> {
        match memory {
            ...
            MemoryKind::REGISTER => Ok(self.registers[index as usize].as_value()),
    ...
    fn get_i64(&self, Address { memory, index }: Address) -> Result<i64, VmError> {
        match memory {
            ...
            MemoryKind::REGISTER => {
                let low_bits = self.registers[index as usize].as_bits() as i64;
                let high_bits = self.registers[index as usize + 1].as_bits() as i64;
    ...
    fn get_i128(&self, Address { memory, index }: Address) -> Result<i128, VmError> {
        match memory {
            ...
            MemoryKind::REGISTER => {
                let low_bits = self.registers[index as usize].as_bits() as i128;
                let low_mid_bits = self.registers[index as usize + 1].as_bits() as i128;
                let high_mid_bits = self.registers[index as usize + 2].as_bits() as i128;
                let high_bits = self.registers[index as usize + 3].as_bits() as i128;
                let full_bits =
    ...
    fn get_u64(&self, Address { memory, index }: Address) -> Result<u64, VmError> {
        match memory {
            ...
            MemoryKind::REGISTER => {
                let low_bits = self.registers[index as usize].as_bits() as u64;
                let high_bits = self.registers[index as usize + 1].as_bits() as u64;
    ...
    fn get_u128(&self, Address { memory, index }: Address) -> Result<u128, VmError> {
        match memory {
            ...
            MemoryKind::REGISTER => {
                let low_bits = self.registers[index as usize].as_bits() as u128;
                let low_mid_bits = self.registers[index as usize + 1].as_bits() as u128;
                let high_mid_bits = self.registers[index as usize + 2].as_bits() as u128;
                let high_bits = self.registers[index as usize + 3].as_bits() as u128;
    ...
    fn add(&mut self, instruction: Instruction) -> Result<(), VmError> {
        ...
        match operand_type {
            ...
            OperandType::F_64 => {
                ...
                self.set_f64_to_registers(sum, destination)?;
            }
            _ => return Err(VmError::UnsupportedOperandType { operand_type }),
        }
    ...
    fn subtract(&mut self, instruction: Instruction) -> Result<(), VmError> {
        ...
        match operand_type {
            ...
            OperandType::I_64 => {
                ...
                self.set_i64_to_registers(difference, destination)?;
            }
            OperandType::I_128 => {
                let left = self.get_i128(left_address)?;
                let right = self.get_i128(right_address)?;
                ...
                self.set_i128_to_registers(difference, destination)?;
            }
            OperandType::U_8 => {
                let left = self.get_u8(left_address)?;
                let right = self.get_u8(right_address)?;
            ...
            OperandType::U_64 => {
                ...
                self.set_u64_to_registers(difference, destination)?;
            }
            OperandType::U_128 => {
                let left = self.get_u128(left_address)?;
                let right = self.get_u128(right_address)?;
                ...
                self.set_u128_to_registers(difference, destination)?;
            }
            OperandType::F_32 => {
                let left = self.get_f32(left_address)?;
                let right = self.get_f32(right_address)?;
            ...
            OperandType::F_64 => {
                ...
                self.set_f64_to_registers(difference, destination)?;
            }
            _ => return Err(VmError::UnsupportedOperandType { operand_type }),
        }
    ...
    fn multiply(&mut self, instruction: Instruction) -> Result<(), VmError> {
        ...
        match operand_type {
            ...
            OperandType::I_64 => {
                ...
                self.set_i64_to_registers(product, destination)?;
            }
            OperandType::I_128 => {
                let left = self.get_i128(left_address)?;
                let right = self.get_i128(right_address)?;
                ...
                self.set_i128_to_registers(product, destination)?;
            }
            OperandType::U_8 => {
                let left = self.get_u8(left_address)?;
                let right = self.get_u8(right_address)?;
            ...
            OperandType::U_64 => {
                ...
                self.set_u64_to_registers(product, destination)?;
            }
            OperandType::U_128 => {
                let left = self.get_u128(left_address)?;
                let right = self.get_u128(right_address)?;
                ...
                self.set_u128_to_registers(product, destination)?;
            }
            OperandType::F_32 => {
                let left = self.get_f32(left_address)?;
                let right = self.get_f32(right_address)?;
            ...
            OperandType::F_64 => {
                ...
                self.set_f64_to_registers(product, destination)?;
            }
            _ => return Err(VmError::UnsupportedOperandType { operand_type }),
        }
    ...
    fn divide(&mut self, instruction: Instruction) -> Result<(), VmError> {
        ...
        match operand_type {
            ...
            OperandType::I_64 => {
                ...
                self.set_i64_to_registers(quotient, destination)?;
            }
            OperandType::I_128 => {
                let left = self.get_i128(left_address)?;
                let right = self.get_i128(right_address)?;
                ...
                self.set_i128_to_registers(quotient, destination)?;
            }
            OperandType::U_8 => {
                let left = self.get_u8(left_address)?;
                let right = self.get_u8(right_address)?;
            ...
            OperandType::U_64 => {
                ...
                self.set_u64_to_registers(quotient, destination)?;
            }
            OperandType::U_128 => {
                let left = self.get_u128(left_address)?;
                let right = self.get_u128(right_address)?;
                ...
                self.set_u128_to_registers(quotient, destination)?;
            }
            OperandType::F_32 => {
                let left = self.get_f32(left_address)?;
                let right = self.get_f32(right_address)?;
            ...
            OperandType::F_64 => {
                ...
                self.set_f64_to_registers(quotient, destination)?;
            }
            _ => return Err(VmError::UnsupportedOperandType { operand_type }),
        }
    ...
    fn modulo(&mut self, instruction: Instruction) -> Result<(), VmError> {
        ...
        match operand_type {
            ...
            OperandType::I_64 => {
                ...
                self.set_i64_to_registers(remainder, destination)?;
            }
            OperandType::I_128 => {
                let left = self.get_i128(left_address)?;
                let right = self.get_i128(right_address)?;
                ...
                self.set_i128_to_registers(remainder, destination)?;
            }
            OperandType::U_8 => {
                let left = self.get_u8(left_address)?;
                let right = self.get_u8(right_address)?;
            ...
            OperandType::U_64 => {
                ...
                self.set_u64_to_registers(remainder, destination)?;
            }
            OperandType::U_128 => {
                let left = self.get_u128(left_address)?;
                let right = self.get_u128(right_address)?;
                ...
                self.set_u128_to_registers(remainder, destination)?;
            }
            OperandType::F_32 => {
                let left = self.get_f32(left_address)?;
                let right = self.get_f32(right_address)?;
            ...
            OperandType::F_64 => {
                ...
                self.set_f64_to_registers(remainder, destination)?;
            }
            _ => return Err(VmError::UnsupportedOperandType { operand_type }),
        }
    ...
    fn exponent(&mut self, instruction: Instruction) -> Result<(), VmError> {
        ...
        match operand_type {
            ...
            OperandType::F_64 => {
                ...
                self.set_f64_to_registers(result, destination)?;
            }
            _ => return Err(VmError::UnsupportedOperandType { operand_type }),
        }
